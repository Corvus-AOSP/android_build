...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the CorvusRom-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/corvus/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/corvus/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
